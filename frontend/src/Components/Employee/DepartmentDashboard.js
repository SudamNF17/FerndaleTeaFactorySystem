...
const DepartmentDashboard = () => {
  ...
  const [newDept, setNewDept] = useState({
    name: "",
    employees: 0,
    performanceThisMonth: 50,
    performanceLastMonth: 30,
  });
  ...
  const updateDepartment = async (id) => {
    ...
    if (editingDept.employees < 0)
      return alert("Employees cannot be negative!");
    if (
      editingDept.performanceThisMonth < 0 ||
      editingDept.performanceThisMonth > 100 ||
      editingDept.performanceLastMonth < 0 ||
      editingDept.performanceLastMonth > 100
    ) {
    ...
    try {
      ...
      setDepartments(
        departments.map((d) => (d._id === id ? res.data : d))
      );
      setEditingDept(null);
    } catch (err) {
      console.error(err);
    ...
    <div className="dept-container">
      ...
      <div className="add-form">
        ...
        <input
          ...
          value={newDept.performanceThisMonth}
          onChange={(e) =>
            setNewDept({
              ...newDept,
              performanceThisMonth: parseInt(e.target.value) || 0,
            })
          }
        />
        <input
          ...
          value={newDept.performanceLastMonth}
          onChange={(e) =>
            setNewDept({
              ...newDept,
              performanceLastMonth: parseInt(e.target.value) || 0,
            })
          }
        />
              ...
              <tr key={dept._id}>
                ...
                <td>
                  {editingDept && editingDept._id === dept._id ? (
                    <input
                      type="number"
                      value={editingDept.performanceThisMonth}
                      onChange={(e) =>
                        setEditingDept({
                          ...editingDept,
                ...
                <td>
                  {editingDept && editingDept._id === dept._id ? (
                    <input
                      type="number"
                      value={editingDept.performanceLastMonth}
                      onChange={(e) =>
                        setEditingDept({
                          ...editingDept,
                ...
                <td>
                  {editingDept && editingDept._id === dept._id ? (
                    ...
                    <button onClick={() => setEditingDept(dept)}>
                      <FaEdit /> Edit
                    </button>
                  )}
                </td>
              ...
              <PieChart width={250} height={250}>
                <Pie
                  data={[
                    {
                      name: "This Month",
                      value: dept.performanceThisMonth,
                    },
                    {
                      name: "Last Month",
                      value: dept.performanceLastMonth,
                    },
                  ]}
                  cx="50%"
                  cy="50%"
