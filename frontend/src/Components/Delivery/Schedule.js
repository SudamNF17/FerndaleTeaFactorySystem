...
export default function Schedule() {
  ...
  const filtered = useMemo(() => {
    return items.filter((it) => {
      const okVan = vanFilter
        ? it.van_number.toLowerCase().includes(vanFilter.toLowerCase())
        : true;
      const okStatus = statusFilter ? it.status === statusFilter : true;
      const okWholesaler = wholesalerFilter
        ? it.wholesaler_name
            .toLowerCase()
            .includes(wholesalerFilter.toLowerCase())
        : true;
  ...
  const isOverdue = (it) =>
    new Date(it.expected_date).getTime() < Date.now() &&
    !["Completed", "Canceled"].includes(it.status);
  ...
  const calendarEvents = filtered.map((it) => ({
    ...
    title: `${it.van_number} • ${it.driver_name} • ${it.wholesaler_name} (${
      it.wholesaler_phone || "N/A"
    })`,
  ...
  const vansUsage = {};
  filtered.forEach((it) => {
    vansUsage[it.van_number] = (vansUsage[it.van_number] || 0) + 1;
  });
  const barData = {
    labels: Object.keys(vansUsage),
    datasets: [
      {
        label: "Deliveries per Van",
        data: Object.values(vansUsage),
  ...
  return (
    <div className="schedule-container">
      ...
      <div className="actions">
        ...
        <button title="Download delivery schedule as PDF" onClick={generatePDF}>
          Export PDF 📄
        </button>
        <button
          title="Download delivery schedule as Excel"
          onClick={exportExcel}
        >
          Export Excel 📊
        </button>
        ...
        <button
          title="send delivery report to delivery person"
          onClick={() => {
            const email = prompt(
              "Enter the email of the delivery van you want to send the report to:"
            );
            ...
            <tbody>
              {filtered.map((it) => (
                <tr key={it._id} className={isOverdue(it) ? "overdue" : ""}>
                  ...
                  <td>
                    <button onClick={() => updateStatus(it._id, "Dispatched")}>
                      Dispatch
                    </button>
                    <button onClick={() => updateStatus(it._id, "In Transit")}>
                      In Transit
                    </button>
                    <button onClick={() => updateStatus(it._id, "Completed")}>
                      Done
                    </button>
          ...
          <div className="form-row">
            ...
            >
              ...
              {vans
                ...
                .map((v) => (
                  <option key={v._id} value={v.van_number}>
                    {v.van_number} ({v.name})
                  </option>
                ))}
            </select>
